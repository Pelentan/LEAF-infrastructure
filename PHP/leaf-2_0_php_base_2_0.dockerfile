FROM php:8.0.17RC1-apache-bullseye as base
...
RUN apt-get update
RUN apt-get -y --allow-unauthenticated install unixodbc-dev gnupg2 libodbc1 odbcinst odbcinst1debian2 unixodbc-dev lsb-release
...
RUN chmod 644 /etc/ssl/certs/leaf.key
